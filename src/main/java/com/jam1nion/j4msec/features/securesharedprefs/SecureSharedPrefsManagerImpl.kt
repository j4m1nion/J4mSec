...
internal class SecureSharedPrefsManagerImpl() : SecureSharedPrefsManager {
    companion object{
        private const val TAG = "SecureSharedPrefsManager"
        private const val CHECK_SECRET_KEY = "___check_secret_key___"
    ...
    private fun addTestKey(){
        val key = CryptoUtils.getSecretKey()

        if(!hasKey(CHECK_SECRET_KEY)){
            prefs?.edit {putString(CHECK_SECRET_KEY, CryptoUtils.encrypt(key, CHECK_SECRET_KEY_VALUE))}
        }

        getString(CHECK_SECRET_KEY)
    }

    override fun checkKeyHealth() : SecureSharedPrefsKeyHealth {
        return try {
            val encrypted = prefs?.getString(CHECK_SECRET_KEY, null)
            val key = CryptoUtils.getSecretKey()
            if(encrypted == null){
                prefs?.edit {putString(CHECK_SECRET_KEY, CryptoUtils.encrypt(key, CHECK_SECRET_KEY_VALUE))}
            }
    ...
    override fun putString(key: String, value : String, commit : Boolean){
        try {
            val encrypted = CryptoUtils.encrypt(CryptoUtils.getSecretKey(), value)
    ...
    override fun getString(key: String) : String? {
        return try {
            ...
            CryptoUtils.decrypt(CryptoUtils.getSecretKey(), encrypted)
        }
        catch (ex: UserNotAuthenticatedException){
            _errors.postValue(SecureSharedPrefsErrors.AuthRequired(ex))
            null
